...
We are trying to optimize the total euclidean distance between joint positions over our motion planner trajectory subject to the first and last joint positions not changing and Jerk to be 0 everywhere. Optimizing the path length was chosen since in this context we decided "optimal trajectory" meant shortest path length. The initial and final constraints were chosen to ensure we still move the object from start to goal. And the Jerk constraint was chosen to ensure we did not have any unconstrained variables so the optimizer would run quickly. 
